...
- **オーケストレーターであり、スペシャリストではありません。** Dispatch は、メール アプリや分析アプリになることは_しません_。誰かが「先週のサインアップを要約してください」と尋ねると、Dispatch は A2A 経由で分析エージェントを呼び出し、回答を返します。誰かが「アリスへの返信の下書き」を依頼すると、Dispatch はメール エージェントに電話します。
