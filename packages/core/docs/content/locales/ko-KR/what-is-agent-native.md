...
논쟁은 "에이전트가 UI를 대체한다"는 것이 아닙니다. "**에이전트는 동등한 파트너로서 맨 위에 UI가 있는 애플리케이션 내부에 속합니다**." 에이전트가 제품을 _is_하는 앱이라도 인간이 제품을 감독, 구성 및 조종하려면 여전히 UI가 필요합니다. [Agent Surfaces — Headless](/docs/agent-surfaces#headless)를 참조하세요.
