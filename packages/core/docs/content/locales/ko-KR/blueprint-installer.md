...
`agent-native add`는 파일을 작성해 주는 멍청한 스캐폴더가 **아닙니다**. 선별된 Markdown _통합 청사진_을 stdout으로 내보냅니다. 해당 청사진을 자체 코딩 에이전트(Claude 코드, Codex, …)에 파이프하면 전체 컨텍스트가 포함된 라이브 저장소에 대한 변경 사항이 적용됩니다.
